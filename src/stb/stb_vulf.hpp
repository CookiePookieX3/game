...
public:
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex){
        ...
        if(vkEndCommandBuffer(commandBuffer) != VK_SUCCESS){
            throw std::runtime_error("Failes to  command buffer!");
        }
	}
...
void Vulf::createDescriptorSetLayout(){
	...
	std::array<VkDescriptorSetLayoutBinding, 2> bindings = {uboLayoutBinding, sampleLayoutBinding};
...
void Vulf::createTextureImage(){
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load("textures/image.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	createImageView(textureImage, textureImageView, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
}
...
void Vulf::createUniformBuffers(){
...
void Vulf::createDescriptorSets(){
